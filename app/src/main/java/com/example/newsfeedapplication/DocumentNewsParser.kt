...
class DocumentNewsParser @Inject constructor(private val builder: DocumentBuilder) :
    NewsXmlParser {

    override fun parse(inputStream: InputStream): List<News> {
        inputStream.use {
            ...
            for (i in 0 until (list.length - 1)) {
                ...
                for (j in 0 until (node.childNodes.length - 1)) {
                    ...
                    when (cnode.nodeName) {
                        ...
                        IMAGE_SRC_STR -> news.imageSrc =
                            cnode.attributes.getNamedItem(URL_NODE_NAME).textContent
                        DESCRIPTION_STR -> news.description = cnode.textContent
                        CONTENT_STR -> news.content = cnode.textContent
                        CREATION_TIME_STR -> news.creationTime =
                            Converter.getInstant(cnode.textContent)
